...
import { AdviceSMSSenderImpl } from "./AdviceSMSSenderImpl";

describe("AdviceSMSSenderImpl", function() {
    this.timeout(4000);

    const env = new IntegrationTestsEnvironment();
    let adviceSMSSender: AdviceSMSSenderImpl;
    ...
    beforeEach(() => {
        ...
        adviceSMSSender = env.getContainer().get<AdviceSMSSenderImpl>(TYPES.AdviceSMSSender);
